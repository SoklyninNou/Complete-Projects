    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int i = 0; i < size(); ++i) {
            for (int j = 0; j < size() - 1; ++j) {
                if (tile(j, i).value() == tile(j + 1, i).value()) {
                    return true;
                }
            }
        }
        for (int k = 0; k < size(); ++k) {
            for (int l = 0; l < size() - 1; ++l) {
                if (tile(k, l).value() == tile(k, l + 1).value()) {
                    return true;
                }
            }
        }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        if (y < size() - 1) {
            for (int i = y + 1; i < size(); ++i) {
                if (tile(x, i) == null) {
                    ++tilesMoved;
                } else if (tile(x, i).value() == myValue && !tile(x, i).wasMerged() && !currTile.wasMerged()) {
                    ++tilesMoved;
                    score += 2 * myValue;
                    break;
                } else {
            ...
            board.move(x, y + tilesMoved, currTile);
        }
